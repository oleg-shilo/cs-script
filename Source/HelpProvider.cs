...
namespace csscript
{
    internal class AppArgs
    {
        ...
        public const string nathash = "nathash";
    ...
    internal class HelpProvider
    {
        public static string BuildCommandInterfaceHelp()
        {
            ...
            builder.Append("If a relative file path is specified with single-dot preficx it will be automatically converted onto the absolute path \n");
            builder.Append("with respect to the location of the file containing the directive being resolved.\n");
            ...
            builder.Append("probing directory where the match ing file(s) is found. Be carefull with the wide wildcard as '*.cs' as they may lead to \n");
            builder.Append("unpredictable behavier. For example thay may match everything from the very first probing directory, which is typcally a current \n");
            builder.Append("directory. Using more specific wildcards is arguebly more parctical (e.g. 'utils/*.cs', '*Helper.cs', './*.cs')\n");
            builder.Append("------------------------------------\n");
...
#if !net1
            builder.Append("The directory name can be a wildcard based expression.In such a case all directories matching the pattern will be this \n");
