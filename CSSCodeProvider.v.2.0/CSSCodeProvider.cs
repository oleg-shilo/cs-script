...
{
	static public ICodeCompiler CreateCompiler(string sourceFile, object options)
	{
		if (Path.GetExtension(sourceFile).ToLower() == ".vb")
			...
			return new CSScriptCompilers.CSharpCompiler(options);
		else if (Path.GetExtension(sourceFile).ToLower() == ".cpp")
			return new CSScriptCompilers.CPPCompiler();
		else
            return new CSScriptCompilers.CSCompiler(options);
	}
}
