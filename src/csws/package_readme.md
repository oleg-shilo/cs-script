# CS-Script (Windows Application)

Note, this tool is a Windows desktop application (no console) version of the Dotnet `cs-script.cli` tool.
...
CS-Script allows direct single-step execution of files containing ECMA-compliant C# code (either from the shell or from the application).
...
_`csws` is a native executable. It simply launches the script engine assembly (csws.dll) without the need to invoke .NET launcher `dotnet`._

```txt
css script.cs
```

You can read in more details about the script engine on the product home page: https://github.com/oleg-shilo/cs-script/wiki/CLI-Script-Execution.
